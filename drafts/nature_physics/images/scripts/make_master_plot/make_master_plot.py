...
plt.scatter(eta_tau_ttf_grid[markovian_mask], eta_tau_gamma_grid[markovian_mask], s=20, marker='^', c='black', label='Markovian Langevin Equation')

xhis = np.arange(5e-6, 3e-5, 1e-6)
cubic_ttfs = np.load('../cubic_ttfs.npy')
cubic_gammas = np.load('../cubic_gamma_maxes.npy')
plt.scatter(cubic_ttfs, cubic_gammas, marker='D', s=20, c='b', label='Cubic noise')
